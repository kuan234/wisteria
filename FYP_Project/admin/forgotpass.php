...
function sendemail_verify($email,$password)
{
    ...
    $email_template = "
        ...
        <h4>Hi Admin, Your New Password has been updated. Please use this password to login your account.</h4>
        <br /><br />
...
.social-auth-links {
  margin: 10px 0;
        ...
        if(isset($_POST['submitbtn'])){
            ...
            $checkemail = "SELECT * from admlogin WHERE `emailaddress` = '$email'";
            $result = mysqli_query($connect,"$checkemail");
            if(mysqli_num_rows($result)>0)
            {
                mysqli_query($connect,"UPDATE `admlogin` SET `pw` = '$password' where `emailaddress` = '$email'");
